...
{
    public partial class Frm_Mapping : DevExpress.XtraEditors.XtraForm
    {
        CONFIG CFG;
        string TABLENAME = "Bms_XXX_MarkeRGeniusIntegration_Mapping";
        List<Bms_XXX_MarkeRGeniusIntegration_Mapping> OList = new List<Bms_XXX_MarkeRGeniusIntegration_Mapping>();
        public Frm_Mapping(string HEADERNAME)
        {
            InitializeComponent();
        ...
        private void InitializeData(object sender, EventArgs e)
        {
            ...
            OList = HELPER.DataTableToList<Bms_XXX_MarkeRGeniusIntegration_Mapping>(HELPER.SqlSelectLogo($@"SELECT * FROM {TABLENAME}"));
        ...
        private void sb_SaveToBM_Click(object sender, EventArgs e)
        {
            ...
            {
                Bms_XXX_MarkeRGeniusIntegration_Mapping item = (Bms_XXX_MarkeRGeniusIntegration_Mapping)grv_Mapping.GetRow(i);
                com = new SqlCommand($@"INSERT INTO {TABLENAME} (LogoBranch,PosBranch,Ip) VALUES (@LogoBranch,@PosBranch,@Ip)", CON, TRANSACTION);
                com.Parameters.AddWithValue("@LogoBranch", item.LogoBranch);
                com.Parameters.AddWithValue("@PosBranch", item.PosBranch);
                com.Parameters.AddWithValue("@Ip", item.Ip);
                com.ExecuteNonQuery();
            }
            TRANSACTION.Commit();
            SplashScreenManager.CloseForm(false);
        ...
        private void DeleteRow(object sender, EventArgs e)
        {
            ...
            if (XtraMessageBox.Show("Seçili kayıt silinecek emin misiniz?", "Silme Onayı", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                return;
            Bms_XXX_MarkeRGeniusIntegration_Mapping item = (Bms_XXX_MarkeRGeniusIntegration_Mapping)grv_Mapping.GetFocusedRow();
            if (item.Id == 0)
        ...
        {
            Bms_XXX_MarkeRGeniusIntegration_Mapping item = new Bms_XXX_MarkeRGeniusIntegration_Mapping();
            item.LogoBranch = te_LogoBranch.Text;
            item.PosBranch = te_PosBranch.Text;
            item.Ip = te_Ip.Text;
            OList.Add(item);
